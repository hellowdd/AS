        ...
        <div class="pull-container">
            ...
            <form class="form-horizontal" style="margin-top:-53px">
                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">按本月、本季度展示</label>
                    <div class="col-sm-2">
...
function getSort(){
   if($("#sort").val() == 1){
	    quarterStartDate=getMonthStartDate();
        quarterEndDate=getMonthEndDate();
        queryRank()
    }else{
        quarterStartDate = getQuarterStartDate();
        quarterEndDate = getQuarterEndDate()
        queryRank()
    }
}
...
function queryRank() {
	var reg = {
	    ...
	    $.ajax({
	        ...
	        url: "${pageContext.request.contextPath}/api/queryAllAppInfo",
                ...
                for (var i = 0; i < res.data.length; i++) {
                	...
                	for (var j = 0; j < res.data[i].list.length; j++) {
                        ...
                        if (j < 5) {
                            ...
                            "   <div class='imgdiv'><div class='borimg'><img src='" + res.data[i].list[j].logoApp + "' width='100' height='40'></div><a href='javascript:void(0)' id=" + res.data[i].list[j].id + "><i class='iconfont icon-icon2'></i>详情</a><p>" + "点击量:" + res.data[i].list[j].click_rate + "</p></div>" +
                            "   <div class='tac' title='"+res.data[i].list[j].appName+"'><h6>" + res.data[i].list[j].appName + "" +
                                "  </h6> <div class='appbtn'><a href='javascript:void(0)' class='marl_10'><i class='iconfont icon-fav'></i>收藏</a></div>" +
                                "</div></div></div>";
                        }  else if (j == 5) {
                ...
                $(".appbtn").find("a").bind("click", function() {        
                    ...
                    if ($(this).text() == "收藏") {
                        $.ajax({
                            ...
                            contentType: "application/json",
                            data:JSON.stringify(reg),
                            url: "${pageContext.request.contextPath}" + "/manager/rest/favorite/insertFavorite",
                            success: function(res) {
                                if (res.success) {
                                    layer.alert("收藏成功");
                                    queryRank();
